...
import { Bell, CircleHelp, CreditCard, FileText, MessageSquare, Wifi } from 'lucide-react';
  ...
  return <PortalShell role="subscriber">
    <div className="w-full bg-[#2447b6] px-4 pb-7 pt-7 text-white sm:px-8 lg:px-10">
      <div className="mx-auto w-full max-w-7xl">
        ...
        <section className="overflow-hidden rounded-3xl bg-white text-slate-950 shadow-sm"><div className="flex items-center justify-between border-b border-slate-100 px-4 py-5 sm:px-6"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[#2166f3] text-white"><Wifi size={22} /></span><p className="text-xs font-bold uppercase tracking-widest text-slate-400">Sky-Tech Fiber</p></div><Badge value="Unpaid" /></div><div className="flex items-end justify-between gap-4 px-4 py-6 sm:px-6"><div><h2 className="text-xl font-bold">{name}</h2><p className="mt-1 text-sm text-slate-400">{account?.contact || ''}</p></div><div className="text-right"><p className="text-2xl font-bold text-[#2447b6]">{price}</p><p className="text-xs text-slate-400">Amount to pay</p></div></div><div className="grid grid-cols-3 border-t border-slate-100 bg-[#f8faff] py-5"><div className="flex flex-col items-center gap-2 border-r border-slate-200"><Wifi size={21} className="text-[#2166f3]" /><strong>{account ? 'Unli' : 'Loading'}</strong><span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Data</span></div><div className="flex flex-col items-center gap-2 border-r border-slate-200"><Wifi size={21} className="text-[#2166f3]" /><strong>{account?.speedMbps || ''} Mbps</strong><span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Speed</span></div><div className="flex flex-col items-center gap-2"><CreditCard size={21} className="text-[#2166f3]" /><strong>{price}</strong><span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Monthly</span></div></div></section>
      </div>
    </div>
    <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-8 lg:px-10"><h2 className="text-lg font-bold">How Can We Help You?</h2><div className="mt-4 grid grid-cols-4 gap-2 sm:gap-4">{actions.map(({ label, href, Icon }) => <Link href={href} className="flex min-h-24 flex-col items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-1 py-3 text-center shadow-sm sm:min-h-32 sm:gap-4 sm:p-5" key={label}><span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-50 text-[#2166f3] sm:h-12 sm:w-12"><Icon size={20} /></span><span className="text-[10px] font-semibold sm:text-xs">{label}</span></Link>)}</div><h2 className="mt-7 text-lg font-bold">My Transaction History</h2><Link href="/subscriber/my-transactions" className="mt-4 flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-4"><FileText className="text-[#2447b6]" size={34} /><div><p className="text-xs text-slate-400">Current plan</p><p className="mt-1 text-xl font-bold text-[#2447b6]">{plan}</p></div></div></Link><h2 className="mt-7 text-lg font-bold">My Request History</h2><div className="mt-4 grid gap-3">{requests.map((request) => <Link href="/subscriber/my-tickets" className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" key={request.title}><div><p className="font-bold">{request.title}</p><p className="mt-1 text-sm text-slate-400">{request.date}</p></div><Badge value={request.status} /></Link>)}</div></div>
  </PortalShell>;
}
