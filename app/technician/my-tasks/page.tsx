...
import { Bell, CalendarDays, CheckSquare, Clock3, ChevronLeft, ChevronRight, Wrench } from 'lucide-react';
...
export default function TechnicianTasks() {
  ...
  useEffect(() => {
    Promise.all([fetch('/api/technician/account').then((response) => response.json()), fetch('/api/installations').then((response) => response.json())]).then(([accountResult, installationResult]) => {
      if (accountResult.data) setTechnician(accountResult.data);
      if (Array.isArray(installationResult.data)) {
        const tasks = installationResult.data.map((item: InstallationApiItem) => ({ name: item.subscriber?.name || 'Subscriber', type: item.type.replace(/_/g, ' '), time: item.time, date: item.date.slice(0, 10), status: item.status.replace(/_/g, ' ') === 'In Progress' ? 'In Progress' : item.status.replace(/_/g, ' ') === 'Completed' || item.status.replace(/_/g, ' ') === 'Closed' ? 'Done' : 'Scheduled' } as ScheduledTask));
        setScheduledTasks(tasks);
        if (tasks[0]) { setSelectedDate(tasks[0].date); setMonthDate(new Date(`${tasks[0].date}T12:00:00`)); }
      }
    }).catch(() => undefined);
  }, []);
  ...
  const calendarDays = useMemo(() => {
    ...
    return Array.from({ length: 42 }, (_, index) => {
      ...
      return { day: dayOffset <= 0 ? previousDays + dayOffset : dayOffset > daysInMonth ? dayOffset - daysInMonth : dayOffset, muted: dayOffset <= 0 || dayOffset > daysInMonth, date: date.toISOString().slice(0, 10) };
    });
  }, [monthDate]);
  ...
  return (
    <PortalShell role="technician">
      <div className="bg-[#2447b6] px-5 pb-7 pt-7 text-white sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <div className="mb-6 flex items-center justify-between gap-3">
            ...
            <Link href="/shared/notifications" aria-label="Notifications" className="relative rounded-2xl bg-[#4770d6] p-3"><Bell size={20} /><span className="absolute right-1 top-0 grid h-4 min-w-4 place-items-center rounded-full bg-[#ff3b50] px-1 text-[9px] font-bold">2</span></Link>
          </div>
      ...
      <div className="mx-auto max-w-7xl bg-[#eef4fb] px-4 py-6 sm:px-8 lg:px-10">
        ...
        {selectedTasks.length > 0 && <div className="mt-4 grid gap-3">{selectedTasks.map((task) => <article key={`${task.name}-${task.time}`} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex min-w-0 items-center gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-blue-50 text-[#2166f3]"><CalendarDays size={20} /></span><div className="min-w-0"><h3 className="truncate font-bold">{task.name}</h3><p className="mt-1 text-sm text-slate-400">{task.type} · {task.time}</p></div></div><div className="shrink-0 text-right"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusStyles[task.status]}`}>{task.status}</span><p className="mt-1 text-xs text-slate-400">{task.date}</p></div></article>)}</div>}
      </div>
    </PortalShell>
  );
}
